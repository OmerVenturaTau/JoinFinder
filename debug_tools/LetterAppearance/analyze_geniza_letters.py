...
Database-backed, balanced by manuscript::
    ...
    python debug_tools/LetterAppearance/analyze_geniza_letters.py \
        --manuscripts 50 --pages-per-manuscript 3

Reproducible input list (columns: manuscript_id, xml_path)::

    python debug_tools/LetterAppearance/analyze_geniza_letters.py --input-csv pages.csv
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, help="Default: debug_tools/LetterAppearance/outputs/<source>_letters")
...
def main() -> None:
    ...
    output = (
        args.output_dir
        or (PROJECT_ROOT / f"debug_tools/LetterAppearance/outputs/{effective_source}_letters{output_suffix}")
    ).resolve()
