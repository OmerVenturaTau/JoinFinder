"""
Raw DINOv2 tile/glyph discriminability histograms.
...
- two image-level embeddings per image:
  1. mean-pooled tile DINOv2 features
  2. mean-pooled glyph DINOv2 features
...
def build_dinov2_model(device: torch.device, input_size: int) -> torch.nn.Module:
    ...
    model = tile_spec.model.eval().to(device)
...
def mean_pool_valid_features(
    ...
    *,
    ...
    """Encode [B,N,3,H,W], mean-pooling valid DINOv2 features per batch row."""
    ...
    with torch.no_grad():
        for start in range(0, flat_x.shape[0], encode_chunk_size):
            chunk = flat_x[start : start + encode_chunk_size]
            if chunk.shape[-2:] != (input_size, input_size):
                chunk = F.interpolate(chunk, size=(input_size, input_size), mode="bicubic", align_corners=False)
    ...
    for batch_idx, (collated, original_idxs) in enumerate(loader):
        ...
        for local_idx, original_idx in enumerate(original_idxs):
            ...
            if local_idx in tile_embs:
                row["tile_dinov2"] = tile_embs[local_idx]
            if local_idx in glyph_embs:
                row["glyph_dinov2"] = glyph_embs[local_idx]
...
def write_outputs(
    *,
    ...
    summary_lines = ["# Raw DINOv2 Tile/Glyph Histograms", ""]
    for mode in ("tile_dinov2", "glyph_dinov2"):
...
def main() -> int:
    ...
    parser.add_argument("--dino-input-size", type=int, default=TILE_SIZE)
    parser.add_argument("--device", default="cuda" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--out-dir", default=str(Path(__file__).resolve().parent / "outputs" / "dinov2_tile_glyph_histograms"))
    ...
    dino_model = build_dinov2_model(device, args.dino_input_size)
    ...
    same_cos = {"tile_dinov2": [], "glyph_dinov2": []}
    cross_cos = {"tile_dinov2": [], "glyph_dinov2": []}
    rows: List[Dict[str, Any]] = []
    ...
    print(f"  - {os.path.join(out_dir, 'tile_dinov2_histogram.png')}")
    print(f"  - {os.path.join(out_dir, 'glyph_dinov2_histogram.png')}")
